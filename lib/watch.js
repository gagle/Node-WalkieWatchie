...
Wrapper.prototype._deleteFile = function (){
	console.log ("df", this._path);
	this.unwatch ();
	this.emit ("delete", this._path, false);
};

Wrapper.prototype._modifyFile = function (){
	console.log ("mf", this._path);
	this.emit ("modify", this._path);
};
			...
			if (me._isDir){console.log("raw dir:",event, entry)
				...
				if (event === "rename"){
					...
					fs.exists (me._dirname, function (exists){
						exists ? me._modifyFile () : me._deleteFile ();
					});
				}
			}
		});
