...
Wrapper.prototype.watch = function (){
	...
	this._watcher.on ("error", function (error){
		...
		if (WINDOWS && error.code === "EPERM"){
			me.unwatch ();
			//Emit an eperm event because at this level it's not possible to
			//check whether the deleted directory is the main watched directory
			//Because in that case a delete event should be emitted
			me.emit ("eperm");
			return;
		}
		me.emit ("error", error);
	...
	this._watcher.on ("change", function (event, entry){
		if (event === "rename"){
			...
			if (me._root && me._root.entry !== entry) return;
