...
Watcher.prototype._listen = function (watcher){
	...
	if (WINDOWS){
		watcher.on ("eperm", function (){
			//Check if the deleted directory is the main watched directory
			fs.exists (me._dir, function (exists){
				if (exists) return;
				
				me.unwatch ();
				me.emit ("delete", me._dir, true);
				me.emit ("any");
			});
		});
	}
...
Watcher.prototype._watch = function (p, action){
	var next = function (p, t, w, firstMove, cb){
		...
		var fileNext = function (stats){
			...
			me._watchFile (p, { increment: !action });
			cb (false);
		};
		...
		var dirNext = function (stats, first){
			fs.readdir (p, function (error, entries){
				...
				if (!remaining){
					var wrapper = me._watchDirectory (p, { increment: !action });
					if (first){
